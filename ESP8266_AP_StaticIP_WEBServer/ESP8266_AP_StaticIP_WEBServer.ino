...
//  digitalWrite (LED_BUILTIN, 0); //turn the built in LED on pin DO of NodeMCU on
  bool DesiredState;
  (server.arg("led").toInt() == 1) ? DesiredState = LOW : DesiredState = HIGH;
 ...
 /* Dynamically generate the LED toggle link, based on its current state (on or off)*/
  ...
  if (reedState) {
    strcpy(reedText, "REED switch is ON.");
    myservo.write(90);
  } else {
    strcpy(reedText, "REED switch is OFF.");
    myservo.write(0);
  }
...
"<html>\
  ...
  <body>\
    ...
    <p>%s<p>\
    <p>%s<p>\
    <p>This page refreshes every 2 seconds. Click <a href=\"javascript:window.location.reload();\">here</a> to refresh the page now.</p>\
  </body>\
</html>",
